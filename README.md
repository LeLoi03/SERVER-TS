# Hướng dẫn cài đặt và triển khai ứng dụng Node.js với Nginx Reverse Proxy và SSL/TLS

Hướng dẫn này mô tả các bước cần thiết để cài đặt và triển khai một ứng dụng Node.js với Nginx làm reverse proxy và sử dụng Let's Encrypt cho SSL/TLS.  Hướng dẫn này sử dụng `apt` trên hệ thống Debian/Ubuntu.

## I. Cài đặt các công cụ và thư viện cần thiết

### 1. Cập nhật hệ thống và cài đặt curl
...
### 3. Cài đặt Microsoft Edge (Tùy chọn)

Nếu bạn cần cài đặt Microsoft Edge, hãy làm theo các bước sau:
...
## II. Cấu hình Systemd Services

Tạo các file service systemd để quản lý ứng dụng Node.js.  Thay đổi các đường dẫn và tên user cho phù hợp.

### 1. Tạo Client Frontend service `client.service`

```bash
sudo nano /etc/systemd/system/client.service
```

Nội dung file `client.service`:

```ini
[Unit]
Description=Client Application (Frontend)
...
### 3. Tạo Server Backend service `server-ts.service`

```bash
sudo nano /etc/systemd/system/server-ts.service
```

Nội dung file `server-ts.service`:

```ini
[Unit]
Description=SERVER-TS (Backend API)
After=network.target
...
### 4. Quản lý các service systemd

Sau khi tạo các file service, thực hiện các lệnh sau:

```bash
sudo systemctl daemon-reload
sudo systemctl enable client.service  # Khởi động service khi boot hệ thống
sudo systemctl enable server-ts.service  # Khởi động service khi boot hệ thống
...
# Kiểm tra trạng thái (status) của các service
sudo systemctl status client.service
sudo systemctl status server-ts.service

# Xem logs của các service
sudo journalctl -u client.service -f  # Theo dõi log theo thời gian thực
sudo journalctl -u server-ts.service -f  # Theo dõi log theo thời gian thực
```

## III. Cài đặt và cấu hình Nginx (Reverse Proxy)
...
Dán nội dung sau vào file (thay `localhost:8386` và `localhost:3001` nếu ứng dụng của bạn không chạy trên localhost hoặc dùng cổng khác):
