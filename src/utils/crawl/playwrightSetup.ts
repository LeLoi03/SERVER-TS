...
import { chromium, Browser, BrowserContext, Route, Request } from "playwright";
import logToFile from '../logger';
import { PlaywrightSetupResult } from "../../types/crawl.types";

const configService = container.resolve(ConfigService);


// --- Lấy cấu hình từ ConfigService ---
const PLAYWRIGHT_CHANNEL = configService.config.PLAYWRIGHT_CHANNEL;
const PLAYWRIGHT_HEADLESS = configService.config.PLAYWRIGHT_HEADLESS;
const USER_AGENT = configService.config.USER_AGENT;

// Log cảnh báo nếu dùng fallback model
if (!configService.config.PLAYWRIGHT_CHANNEL || !configService.config.PLAYWRIGHT_HEADLESS || !configService.config.USER_AGENT) {
...
export const setupPlaywright = async (): Promise<PlaywrightSetupResult> => {
    ...
    try {
        browser = await chromium.launch({
            ...
            headless: PLAYWRIGHT_HEADLESS, // Sử dụng biến HEADLESS từ config.ts
            args: [
        ...
        await browserContext.route("**/*", (route: Route) => { // Route được sử dụng ở đây
            try {
                const request: Request = route.request(); // Request được sử dụng ở đây
