      ...
      if (issnToSearch){
        ...
        if (issnMatch) {
            ...
            try {
              ...
              await page.waitForSelector('div.p7sI2.PUxBg', { timeout: 10000 });

              // Lấy giá trị thuộc tính `src` của thẻ `<img>` trong `div[jsname="figiqf"]`
              const imageSrc = await page.evaluate(() => {
                const imgElement = document.querySelector('div.p7sI2.PUxBg');
...
const processPage = async (page, url) => {
  try {
    ...
    // console.log('Dữ liệu bảng đã lấy được:', tableData);

    // Xử lý dữ liệu Bioxbio sau khi lấy từ bảng
    // for (const row of tableData) {
    //   if (row.journalName) {
    //     console.log(`Tìm kiếm thông tin Bioxbio cho tạp chí: ${row.journalName}`);
    //     const bioxbioSearchUrl = `https://www.bioxbio.com/?q=${encodeURIComponent(row.journalName)}`;
    //     console.log(`URL tìm kiếm Bioxbio: ${bioxbioSearchUrl}`);
    //     row.bioxbioData = await fetchBioxbioData(page, bioxbioSearchUrl, row.journalName);
    //     console.log(`Dữ liệu Bioxbio cho ${row.journalName}:`, row.bioxbioData);
    //   }
    // }
...
(async () => {
  ...
  const urls = Array.from({ length: 1 }, (_, i) => `${BASE_URL}&page=${i + 1}`);
  const csvResults = [];
  const jsonResults = [];
  ...
  fs.writeFileSync(OUTPUT_CSV, CSV_HEADERS + csvResults.join('\n'), 'utf8');
