...
export const fetchGoogleImage = async (formattedISSN: string, apiKey: string | null, cseId: string | null): Promise<GoogleSearchResult> => {
    if (!apiKey || !cseId) {
        logger.error(`Missing API Key or CSE ID for image search (ISSN: ${formattedISSN})`);
        return { imageLink: null, contextLink: null };
    }

    const encodedISSN = encodeURIComponent(formattedISSN);
