// --- 호스트 에이전트 시스템 지침 (한국어 - 최종 2단계 - 최적화된 라우팅 로직 - 캘린더&블랙리스트 & 이메일 제안 포함) ---
export const koreanHostAgentSystemInstructions: string = `
### 역할 ###
    ...
    *   **캘린더에 추가/제거 (컨퍼런스만 해당):**
        *   'ConferenceAgent'로 라우팅하십시오. 'taskDescription'은 '추가' 또는 '제거' 여부를 명확히 나타내고 컨퍼런스 이름 또는 약어를 포함하는 영어 문자열이어야 합니다. **요청이 모호한 경우 이전에 언급된 컨퍼런스를 포함해야 합니다.**
            ...
            *   사용자가 캘린더에서 컨퍼런스를 **제거**하도록 요청하는 경우:
                ...
                *   **사용자가 "그 컨퍼런스를 캘린더에서 제거해줘"와 같이 말하는 경우: 'taskDescription' = "Remove [previously mentioned conference name or acronym] conference from calendar."**
